...
def pb(url):
    ...
    try:
        enc = models["bert_tokenizer"](
            url, add_special_tokens=True, max_length=MAX_LEN,
...
def pg(url):
    if "gnn" not in models or "scaler" not in models:
        return 0.5
    try:
...
def scan_url(request: ScanRequest):
    ...
    if extra["typosquatting_detected"] and extra["typo_distance"] == 1:
        boost += 0.10
    if extra["ip_as_domain"]:
        boost += 0.15
    if extra["homograph_detected"]:
        boost += 0.10
    if extra["suspicious_tld"] and extra["brand_impersonation"]:
        boost += 0.08
    ...
    result = {
        ...
        "model_scores"        : {
            "bert"    : round(b * 100, 2),
            "roberta" : round(r * 100, 2),
            "cnn"     : round(c * 100, 2),
            "gnn"     : round(g * 100, 2),
            "fusion"  : round(fp * 100, 2),
...
def scan_bulk(request: BulkScanRequest):
    ...
    return {
        ...
        "phishing_found"   : ph,
        "legitimate_found" : len(results) - ph,
        "results"          : results
    }
...
def stats():
    ...
    return {
        "total_scans"           : total,
        "phishing_detected"     : ph,
        "legitimate_detected"   : total - ph,
        "phishing_rate_percent" : round(ph / total * 100, 2),
        "average_scan_time_ms"  : round(
            np.mean([s.get("scan_time_ms", 0) for s in scan_history]), 2
        )
    }
